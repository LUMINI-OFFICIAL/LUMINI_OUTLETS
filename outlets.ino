...
static const unsigned long transmissionInterval = 1000;

float sensitivity = 0.66; // check sensitivity chart
float mCurrent = 30;      // 30A
int noise = 3; // Normally 1, always calibrate
...
void transmissionLoop(){
  cReading = getReading();
  if (millis() - lastSendTime > transmissionInterval) {
...
float getReading()
{
  ...
  currentReading = ((voltage - vOffset) / sensitivity)+noise;
